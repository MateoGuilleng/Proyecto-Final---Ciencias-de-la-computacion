...
public class ControlPrincipal {
    ...
    /**
     * Registra una nueva solicitud. CRITICAS van a colaCriticas, ORDINARIAS a
     * colaOrdinarias.
     * 
    ...
    public String asignarRecursosASiguiente(String tecnicoId, String unidadId, boolean usarKit) {
        ...
        if (sol == null)
            return "Error: No hay solicitudes pendientes en cola.";

        Tecnico tec = buscarTecnico(tecnicoId);
        if (tec == null)
            return "Error: Técnico no encontrado.";
        if (!tec.isDisponible())
            return "Error: El técnico no está disponible.";

        UnidadServicio uni = buscarUnidad(unidadId);
        if (uni == null)
            return "Error: Unidad no encontrada.";
        if (!uni.isDisponible())
            return "Error: La unidad no está disponible.";
        if (uni.getEstado() == EstadoUnidad.MANTENIMIENTO)
            return "Error: La unidad está en mantenimiento.";

        Kit kit = null;
        if (usarKit) {
            if (pilaKitsDisponibles.estaVacia())
                return "Error: No hay kits disponibles en la pila.";
        ...
        if (sol.getPrioridad() == Prioridad.CRITICA)
            colaCriticas.desencolar();
        else
            colaOrdinarias.desencolar();
    ...
    public String completarServicio(String solicitudId) {
        ...
        if (!sol.tieneRecursosAsignados())
            return "Error: La solicitud no tiene recursos asignados.";
    ...
    public Kit verKitDisponibleEnCima() {
        return pilaKitsDisponibles.cima();
    }
    ...
    public co.udistrital.modelo.estructuras.Pila<Kit> getPilaKitsDisponibles() {
        return pilaKitsDisponibles;
    }

    /** @return El kit en la cima de la pila de revisión, o null. */
    public Kit verKitRevisionEnCima() {
        return pilaKitsRevision.cima();
    }
    ...
    public co.udistrital.modelo.estructuras.Pila<Kit> getPilaKitsRevision() {
     ...
     * Formato: CLIENTE,nombre,tel | TECNICO,nombre,esp | UNIDAD,tipo,zona | KIT |
     * SOLICITUD,clienteId,desc,prioridad
     */
            ...
            while ((linea = br.readLine()) != null) {
                ...
                try {
                    switch (p[0].trim().toUpperCase()) {
                        ...
                        case "TECNICO":
                            if (p.length >= 3) {
                                registrarTecnico(p[1].trim(), p[2].trim());
                                tec++;
                            } else
                                err++;
                            break;
                        ...
                        case "SOLICITUD":
                            if (p.length >= 4) {
                                SolicitudServicio s = registrarSolicitud(p[1].trim(), p[2].trim(),
                                        Prioridad.valueOf(p[3].trim().toUpperCase()));
                                if (s != null)
                                    sol++;
                                else
                                    err++;
